...
- **Priority-Based Random Sampling**: While the availability of nodes is random (depending on user workloads), the selection process is strictly priority-based. The framework identifies all currently free nodes and prioritizes them based on the validity of their prior test results. Nodes with stale or missing data are tested first, transforming opportunistic sampling into systematic coverage over time.
