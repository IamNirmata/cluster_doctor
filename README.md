...
- Lambda's online validation ( Node Problem Detector for example) is not at application level ( DL unit test, loop back NCCL test, storage test etc), so we need this framework to fill the gap.
