...
# gpu_pods_age_report.sh
# Requires: kubectl, jq, coreutils (date, sort, head, awk)
#
# What it does:
#  1) Lists ONLY pods across all namespaces that request/limit nvidia.com/gpu (>0)
#  2) Computes stats on pod age (avg, stddev, min/max, median, p90, p99)
#  3) Prints Top 50 oldest GPU pods
...
if [[ "$count" == "0" ]]; then
  echo "No GPU pods found (resource key: $RESOURCE_KEY)."
  exit 0
fi

echo "GPU pods found: $count  (resource key: $RESOURCE_KEY)"
echo
...
get_pct() {
  ...
  # Ensure rank in [1,n]
  local rank
  rank="$(awk -v p="$pct" -v n="$n" 'BEGIN{r=int((p/100.0)*n); if((p/100.0)*n>r) r++; if(r<1) r=1; if(r>n) r=n; print r;}')"
